...
// ## As you add more slaves, you will need more of these signals
    wire        HSEL_rom, HSEL_ram, HSEL_dummy;
    
// Slave output signals (one per slave)
// ## As you add more slaves, you will need more of these signals
    wire [31:0] HRDATA_rom, HRDATA_ram;     // read data from each slave
    wire        HREADYOUT_rom, HREADYOUT_ram, HREADYOUT_dummy;   // ready output from each slave
    wire        HRESP_dummy;  // some slaves uses HRESP to signal an error response
    
    // ours (might need this later... FIXME)
...
// ## As you add more slaves, you need to connect more HRDATA and HREADYOUT signals  
    ...
    // Connect the response signals to the response multiplexer
		...
		.HRESP_S2    (OKAY), 
        .HRESP_S3    (OKAY), // No need for UART HRESP             
        .HRESP_S4    (OKAY),
        .HRESP_S5    (OKAY),
...
/* ## Instantiate the GPIO block: AHBgpio.  Connect its bus signals to the bus signals - including
   ...
   output signals to switches, buttons and LEDs as explained in the assignment instructions. */
    AHBgpio GPIO(
               ...
               // Output GPIO signas (all are [15:0])
               .gpio_out0   (led_gpio),    // read-write address 0 
               //.gpio_out1   (BAD_DATA),    // read-write address 4 <------------- this might be important
               // Input GPIO signas (all are [15:0])
               .gpio_in0    (sw),        // read only address 8 (sw is the 16 switches)
               // TODO FIXME
               .gpio_in1    (buttons)        // read only address C (buttons = 5 pushbuttons)
       );

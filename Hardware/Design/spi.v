...
module spi(
    ...
    // === Other registers === 
    reg [3:0] countClk = 5'b0; // counts the clock edges
    ...
    always @ (currentState, sclk_rising, sclk_falling) // mealy meachine so inputs are included 
      case (currentState)
        IDLE:  begin
                  ...
                  update_mosi = 1'b0;
                  read_miso = 1'b0;
                  dataRx_ready = 1'b0; // when high, byte is ready to be read, when low it is the byte from the previus transaction
              end
...
always @ (read_miso, countBit)
    begin
         if (read_miso) nextCountBit = countBit + 5'b00001;
